...
export const weatherRouter = createTRPCRouter({
    getWeather: publicProcedure
        ...
        .query(async ({ input }) => {
            const response = await fetch(
                `https://api.openweathermap.org/data/2.5/weather?lat=${input.lat}&lon=${input.lon}&appid=${env.OPEN_WEATHER_API_KEY}`
            );
    ...
    getLatLongFromCityAndState: publicProcedure
        ...
        .query(async ({ input }) => {
            const response = await fetch(
                `https://api.openweathermap.org/data/2.5/weather?q=${input.city},${input.state}&appid=${env.OPEN_WEATHER_API_KEY}`
            );
